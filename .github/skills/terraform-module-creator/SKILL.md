...
Always create an `examples/basic/` folder alongside the module. This example should demonstrate the minimum viable usage — what a consumer needs to write to call the module with sensible defaults. For modules with distinct usage patterns (e.g., with and without private endpoint), add a second subfolder such as `examples/private-endpoint/`. The example is also what terraform-docs uses to populate a usage block in the README.
...
- **`examples/basic/main.tf`** — minimum viable usage of the module with sensible defaults; add further subfolders for distinct usage patterns (e.g., `examples/private-endpoint/`)
